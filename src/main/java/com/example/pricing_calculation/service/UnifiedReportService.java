...
public class UnifiedReportService {
    ...
    public ManagerReportResponse report(LocalDate from, LocalDate to) {
        ...
        List<VehicleTypeSummary> byType = types.findAll().stream().map(t -> {
            long en = range.stream().filter(s -> s.getVehicle().getVehicleType().getId().equals(t.getId())).count();
            long ex = slist.stream()
                    .filter(s -> s.getExitTime() != null && !s.getExitTime().isBefore(start)
                            && s.getExitTime().isBefore(end)
                            && s.getVehicle().getVehicleType().getId().equals(t.getId()))
                    .count();
            long parked = slist.stream().filter(s -> List.of("ACTIVE", "PAYMENT_PENDING").contains(up(s.getStatus()))
                    && s.getVehicle().getVehicleType().getId().equals(t.getId())).count();
            List<PaymentModuleParkingSlot> ts = slotList.stream()
                    .filter(s -> s.getZone().getVehicleType().getId().equals(t.getId())).toList();
            BigDecimal rev = payments.findAll().stream()
                    .filter(p -> List.of("COMPLETED", "SUCCESS").contains(up(p.getStatus()))
                            && p.getPaymentTime() != null && !p.getPaymentTime().isBefore(start)
                            && p.getPaymentTime().isBefore(end)
                            && p.getSession().getVehicle().getVehicleType().getId().equals(t.getId()))
        ...
        return new ManagerReportResponse(start, end, range.size(), exits, current,
                payments.sumCompletedAmountBetween(start, end), slotList.size(), count(slotList, "AVAILABLE"), occupied,
                count(slotList, "RESERVED"), count(slotList, "MAINTENANCE"), count(slotList, "LOCKED"),
                slotList.isEmpty() ? 0 : occupied * 100.0 / slotList.size(), peak, byType);
    }
