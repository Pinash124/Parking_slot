...
public class PaymentModuleAuthService {
    ...
    @Transactional public AuthRegistrationResponse registerDirect(AuthRegistrationRequest r){validateRegistration(r);String email=email(r.email());if(users.existsByEmailIgnoreCase(email))throw new BadRequestException("Email is already registered");UserAccount u=new UserAccount();u.setFullName(r.fullName());u.setEmail(email);u.setPhone(r.phone());u.setPasswordHash(passwords.hash(r.password()));u.setStatus("ACTIVE");u.setRole(UserRole.PARKING_USER.code());return registrationResponse(users.save(u),"Registration completed");}
    ...
    @Transactional public AuthLogoutResponse changePassword(String header,ChangePasswordRequest r){UserAccount u=authenticate(header);if(r==null||!passwords.matches(r.currentPassword(),u.getPasswordHash()))throw new UnauthorizedException("Current password is incorrect");validatePassword(r.newPassword());if(passwords.matches(r.newPassword(),u.getPasswordHash()))throw new BadRequestException("New password must be different");u.setPasswordHash(passwords.hash(r.newPassword()));users.save(u);revokeUser(u.getId());return new AuthLogoutResponse("Password changed; please login again");}
    ...
    private void validatePassword(String p){if(p==null||p.length()<8||p.length()>128)throw new BadRequestException("password must contain between 8 and 128 characters");}
