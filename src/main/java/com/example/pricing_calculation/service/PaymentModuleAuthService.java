...
public class PaymentModuleAuthService {
    ...
    private final UserAccountRepository users; private final PasswordHashService passwords; private final OtpDeliveryService otpDelivery; private final AuthAccessSessionRepository sessions;
    ...
    public PaymentModuleAuthService(UserAccountRepository users,PasswordHashService passwords,OtpDeliveryService otpDelivery,AuthAccessSessionRepository sessions){this.users=users;this.passwords=passwords;this.otpDelivery=otpDelivery;this.sessions=sessions;}

    public OtpChallengeResponse requestRegistration(AuthRegistrationRequest r){validateRegistration(r);String email=email(r.email());if(users.existsByEmailIgnoreCase(email))throw new BadRequestException("Email is already registered");String phone=validatePhone(r.phone());String otp=otp();LocalDateTime expires=LocalDateTime.now().plusMinutes(OTP_MINUTES);registrations.put(email,new PendingRegistration(r.fullName().trim(),usernameOrDefault(r.username(),r.fullName(),email),email,phone,passwords.hash(r.password()),otp,expires,0));otpDelivery.send(email,otp,"REGISTER");return challenge(email,"REGISTER",expires,otp);}
    @Transactional public AuthRegistrationResponse verifyRegistration(VerifyOtpRequest r){String email=requiredEmail(r==null?null:r.email());PendingRegistration p=registrations.get(email);verify(p==null?null:p.otp(),p==null?null:p.expiresAt(),p==null?0:p.attempts(),r==null?null:r.otp(),()->registrations.computeIfPresent(email,(k,v)->v.moreAttempts()));if(users.existsByEmailIgnoreCase(email))throw new BadRequestException("Email is already registered");UserAccount u=new UserAccount();u.setFullName(p.fullName());u.setUsername(p.username());u.setEmail(email);u.setPhone(p.phone());u.setPasswordHash(p.passwordHash());u.setStatus("ACTIVE");u.setRole(UserRole.PARKING_USER.code());u.setCreatedAt(LocalDateTime.now());registrations.remove(email);return registrationResponse(users.save(u),"Registration completed");}
    ...
    @Transactional public AuthLoginResponse verifyLogin(VerifyOtpRequest r){String email=requiredEmail(r==null?null:r.email());OtpState state=loginOtps.get(email);verify(state==null?null:state.otp(),state==null?null:state.expiresAt(),state==null?0:state.attempts(),r==null?null:r.otp(),()->loginOtps.computeIfPresent(email,(k,v)->v.moreAttempts()));UserAccount u=users.findById(state.userId()).orElseThrow(()->new UnauthorizedException("User not found"));loginOtps.remove(email);return issue(u);}

    @Transactional public AuthRegistrationResponse registerDirect(AuthRegistrationRequest r){validateRegistration(r);String email=email(r.email());if(users.existsByEmailIgnoreCase(email))throw new BadRequestException("Email is already registered");String phone=validatePhone(r.phone());UserAccount u=new UserAccount();u.setFullName(r.fullName());u.setUsername(usernameOrDefault(r.username(),r.fullName(),email));u.setEmail(email);u.setPhone(phone);u.setPasswordHash(passwords.hash(r.password()));u.setStatus("ACTIVE");u.setRole(UserRole.PARKING_USER.code());u.setCreatedAt(LocalDateTime.now());return registrationResponse(users.save(u),"Registration completed");}
    @Transactional public AuthLoginResponse loginDirect(AuthLoginRequest r){return issue(credentials(r));}
    ...
    @Transactional public UserProfileResponse updateProfile(String header,UpdateProfileRequest r){UserAccount u=authenticate(header);if(r!=null&&r.username()!=null)u.setUsername(validateUsername(r.username(),u.getFullName(),u.getEmail()));if(r!=null&&r.fullName()!=null&&!r.fullName().isBlank())u.setFullName(r.fullName());if(r!=null&&r.phone()!=null)u.setPhone(validatePhone(r.phone()));return profile(users.save(u));}
    @Transactional public AuthLoginResponse loginWithGoogle(String email,String fullName){String normalized=requiredEmail(email);UserAccount u=users.findByEmailIgnoreCase(normalized).orElseGet(()->{UserAccount n=new UserAccount();String resolvedName=fullName==null||fullName.isBlank()?normalized:fullName;n.setEmail(normalized);n.setFullName(resolvedName);n.setUsername(usernameOrDefault(null,resolvedName,normalized));n.setPasswordHash(passwords.hash(generateToken()));n.setStatus("ACTIVE");n.setRole(UserRole.PARKING_USER.code());n.setCreatedAt(LocalDateTime.now());return users.save(n);});if(!"ACTIVE".equalsIgnoreCase(u.getStatus()))throw new UnauthorizedException("Account is not active");return issue(u);}

    @Transactional public AuthLogoutResponse logout(String header){String tokenHash=hashToken(extract(header));AuthAccessSession session=sessions.findById(tokenHash).orElseThrow(()->new UnauthorizedException("Invalid or expired access token"));sessions.delete(session);return new AuthLogoutResponse("Logout completed");}
    @Transactional(readOnly=true) public UserAccount authenticate(String header){AuthAccessSession s=sessions.findById(hashToken(extract(header))).orElseThrow(()->new UnauthorizedException("Invalid or expired access token"));if(s.getExpiresAt().isBefore(LocalDateTime.now()))throw new UnauthorizedException("Invalid or expired access token");return users.findById(s.getUserId()).orElseThrow(()->new UnauthorizedException("Invalid or expired access token"));}
    ...
    private AuthLoginResponse issue(UserAccount u){purge();String raw=generateToken();LocalDateTime now=LocalDateTime.now();LocalDateTime expires=now.plusHours(SESSION_HOURS);sessions.save(new AuthAccessSession(hashToken(raw),u.getId(),now,expires));return new AuthLoginResponse(raw,"Bearer",expires,u.getId(),u.getUsername(),u.getFullName(),u.getEmail(),UserRole.fromCode(u.getRole()).code());}
    ...
    private UserProfileResponse profile(UserAccount u){return new UserProfileResponse(u.getId(),u.getUsername(),u.getFullName(),u.getEmail(),u.getPhone(),u.getStatus(),UserRole.fromCode(u.getRole()).code());}
    private String usernameOrDefault(String username,String fullName,String email){return validateUsername(username==null||username.isBlank()?fullName:username,fullName,email);}
    private String validateUsername(String username,String fullName,String email){String value=username==null?"":username.trim();if(value.isEmpty())value=fullName==null||fullName.isBlank()?email:fullName.trim();if(value.length()<2||value.length()>50)throw new BadRequestException("username must be 2-50 characters");return value;}
    private String validatePhone(String phone){String value=phone==null?"":phone.trim();if(!value.matches("\\d{10}"))throw new BadRequestException("phone must be exactly 10 digits");return value;}
    ...
    private record PendingRegistration(String fullName,String username,String email,String phone,String passwordHash,String otp,LocalDateTime expiresAt,int attempts){PendingRegistration moreAttempts(){return new PendingRegistration(fullName,username,email,phone,passwordHash,otp,expiresAt,attempts+1);}}
